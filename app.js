...
function toolCode(a){
  return a.tool_code ?? a.asset_code ?? a.code ?? '';
}

function toolName(a){
  return a.tool_name ?? a.asset_name ?? a.name ?? '';
}

function toolCategory(a){
  return a.category ?? a.asset_type ?? '';
}

function toolDept(a){
  return a.department_name ?? a.department ?? a.dept ?? deptNameById(a.department_id) ?? '';
}

function toolQty(a){
  return Number(a.quantity ?? a.qty ?? 0);
}

function toolCost(a){
  return Number(a.original_cost ?? a.cost ?? 0);
}
...
async function loadRemote(){
  ...
  try{
    ...
    showToast(
      'Không kết nối được API, đang dùng dữ liệu local',
      'warn',
      'API'
    );
  }
...
function renderWarnings(){
  ...
  $('warningStats').innerHTML = `
    ...
    <div class="stat-row stat-danger">
      <b>⛔ Hỏng / mất / thanh lý</b>
      <span>${issue}</span>
    </div>

    <div class="stat-row stat-warn">
      <b>⚠️ Chưa gán bộ phận</b>
      <span>${noDept}</span>
    </div>

    <div class="stat-row stat-warn">
      <b>📌 Có chênh lệch kiểm kê</b>
      <span>${diff}</span>
    </div>
  `;
}
...
function renderDept(){
  $('deptGrid').innerHTML = departments.map(d => {
    const list = tools.filter(a => norm(toolDept(a)) === norm(d.name));
    const value = list.reduce((s,a) => s + toolCost(a), 0);
...
async function saveTool(){
  ...
  const localItem = {
    id: editingToolId || Date.now(),
    ...payload
  };

  if(editingToolId){
    tools = tools.map(a => a.id === editingToolId ? localItem : a);
  }else{
    tools.unshift(localItem);
  }
  ...
  showToast(
    editingToolId ? 'Đã cập nhật CCDC' : 'Đã thêm CCDC',
    'success',
    'Thành công'
  );
...
async function deleteTool(id){
  const ok = await showConfirm(
    'Bạn có chắc muốn xóa CCDC này?',
    'Xóa CCDC'
  );

  if(!ok) return;

  tools = tools.filter(a => a.id !== id);

  renderAll();

  await saveRemote('/api/tools/' + id, null, 'DELETE');
...
async function saveAssignment(){
  ...
  if(editingAssignmentId){
    assignments = assignments.map(x => x.id === editingAssignmentId ? item : x);
  }else{
...
async function deleteAssignment(id){
  const ok = await showConfirm('Xóa phiếu này?', 'Xóa phiếu');
  ...
  await saveRemote('/api/assignments/' + id, null, 'DELETE');
}
...
async function saveInventory(){
  ...
  if(editingInventoryId){
    inventories = inventories.map(x => x.id === editingInventoryId ? item : x);
  }else{
...
function renderInventories(){
  $('inventoryRows').innerHTML = inventories.map(i => `
    <tr>
      ...
      <td>${i.action || ''}</td>
      <td>
        <button class="btn danger" onclick="deleteInventory(${i.id})">Xóa</button>
      </td>
    </tr>
...
async function deleteInventory(id){
  const ok = await showConfirm(
    'Xóa dòng kiểm kê này?',
    'Xóa kiểm kê'
  );

  if(!ok) return;

  inventories = inventories.filter(x => x.id !== id);

  renderAll();

  await saveRemote('/api/inventories/' + id, null, 'DELETE');
}
...
function setView(id){
  ...
  const map = {
    dashboard:[
      'Tổng quan CCDC',
      'Theo dõi tổng quan công cụ, dụng cụ và tình trạng sử dụng.'
    ],
    tools:[
      'Danh sách CCDC',
      'Quản lý chi tiết từng công cụ, dụng cụ.'
    ],
    departments:[
      'Bộ phận sử dụng',
      'Cơ cấu phòng ban để gắn CCDC.'
    ],
    assignments:[
      'Cấp phát / Thu hồi',
      'Lịch sử bàn giao công cụ, dụng cụ.'
    ],
    inventories:[
      'Kiểm kê',
      'Ghi nhận kiểm kê năm, số lượng sổ sách và thực tế.'
    ],
    reports:[
      'Báo cáo',
      'Báo cáo kiểm kê và xuất dữ liệu.'
    ],
    settings:[
      'Cấu hình API',
      'Kết nối Cloudflare Worker + D1.'
    ]
  };
...
function renderReportByDept(){
  const rows = departments.map(d => {
    ...
    return {
      ...
      value: list.reduce((s,a) => s + toolCost(a), 0)
    };
  }).filter(r => r.total > 0);
...
async function importExcel(event){
  ...
  try{
    ...
    for(const row of rows){
      ...
      await saveRemote('/api/tools', payload, 'POST', false);
    }
...
function renderDeptChart(){
  ...
  const rows = departments
    .map(d => ({
      name:d.name,
      count:tools.filter(a => norm(toolDept(a)) === norm(d.name)).length
    }))
    .filter(x => x.count > 0)
